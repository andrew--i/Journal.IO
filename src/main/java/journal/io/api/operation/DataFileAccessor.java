...
public class DataFileAccessor {
    ...
    private final ConcurrentMap<Thread, ConcurrentMap<Integer, FileAccess>> perThreadDataFileRafs = new ConcurrentHashMap<>();
    ...
    public void deleteFromLocation(Location source) throws IOException {
        ...
        try {
            if (opened) {
                ...
                //
                FileAccess raf = getOrCreateRaf(Thread.currentThread(), source.getDataFileId());
    ...
    public Location readLocationDetails(int file, int pointer) throws IOException {
        ...
        if (asyncWrite != null) {
            ...
            try {
                if (opened) {
                    FileAccess raf = getOrCreateRaf(Thread.currentThread(), location.getDataFileId());
    ...
    public Location readNextLocationDetails(Location start, final int type) throws IOException {
        ...
        if (asyncWrite != null) {
            ...
            try {
                if (opened) {
                    FileAccess raf = getOrCreateRaf(Thread.currentThread(), start.getDataFileId());
                    if (isIntoNextLocation(raf, start) || seekToLocation(raf, start, true, false)) {
                        if (raf.hasRecordHeader(raf.getCurrentPosition(), journal.isOpened())) {
                            ...
                            do {
                                ...
                                HeaderRecord headerRecord = raf.readRecordHeader();
    ...
    public void close() {
        ...
        try {
            opened = false;
            for (Entry<Thread, ConcurrentMap<Integer, FileAccess>> threadRafs : perThreadDataFileRafs.entrySet()) {
                for (Entry<Integer, FileAccess> raf : threadRafs.getValue().entrySet()) {
    ...
    private boolean isIntoNextLocation(FileAccess raf, Location source) throws CompactedDataFileException, IOException {
    ...
    private boolean seekToLocation(FileAccess raf, Location destination, boolean fillLocation, boolean fillData) throws IOException {
        ...
        if (pointer != destination.getPointer()) {
            ...
            if (pointer != destination.getPointer()) {
                ...
                if (hint != null && hint.getKey().getDataFileId() == destination.getDataFileId()) {
                    ...
                    position = ConfigurationFactory.CONFIGURATION().getFileHeaderSize();
                }
                raf.seek(position);
                if (raf.hasRecordHeader(position, journal.isOpened())) {
                    HeaderRecord headerRecord = raf.readRecordHeader();
                    ...
                    while (pointer != destination.getPointer()) {
                        raf.skip(size - ConfigurationFactory.CONFIGURATION().getRecordHeaderSize());
                        position = raf.getCurrentPosition();
                        if (raf.hasRecordHeader(position, journal.isOpened())) {
                            HeaderRecord headerRecord2 = raf.readRecordHeader();
    ...
    private FileAccess getOrCreateRaf(Thread thread, Integer file) throws IOException {
        ConcurrentMap<Integer, FileAccess> rafs = perThreadDataFileRafs.get(thread);
        if (rafs == null) {
            rafs = new ConcurrentHashMap<>();
            perThreadDataFileRafs.put(thread, rafs);
        }
        FileAccess raf = rafs.get(file);
        if (raf == null) {
            raf = journal.getDataFile(file).createDataAccess();
            raf.skip(ConfigurationFactory.CONFIGURATION().getFileHeaderSize());
            rafs.put(file, raf);
        }
        return raf;
    }
    ...
    private void removeRaf(Thread thread, Integer file) throws IOException {
        FileAccess fileAccess = perThreadDataFileRafs.get(thread).remove(file);
        fileAccess.close();
    }
